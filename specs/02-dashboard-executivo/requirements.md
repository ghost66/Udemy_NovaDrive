# Requeriments - Dashboard Executivo

**Stakeholder** CEO - Ricardo Matos

## Objetivo
Dar ao CEO uma vião rápida e executiva do desempenho de vendas entre concessionárias, para decisão ágil e apresentação trimestral ao conselho

## Contexto do Pedido
Ricardo hoje só sabe como a empresa está indo pelo relatório de fechamento mensal do financeiro, tarde demais para agir. Quer abrir  uma tela e entender rapidamente se está vendendo mais ou menos que antes, quais unidades vão bem ou mal, e se possível uma projeção do mês.
...
## User stories
- O sistema deve exibir faturamento total e quantidadede de veículos vendidos no período selecionado quando o CEO abrir o painel.
...
-**Projeção** : Ricardo autorizou simplificar ou adiar ("pode ficar para depois, mas eu queria"). Decisão do MVP: implemenar uma projeção simples de um run-rate (regra de três sobre o ritmo atual do mês). Alternativa igualmente válida: registrar como item de fase 2.
-**Geração automática de apresentação para o conselho**: fora de escopo. Interpretado como necessidade de visual limpo (gráficos), não geração de slide.
